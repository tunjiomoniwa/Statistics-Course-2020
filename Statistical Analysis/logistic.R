...
fit.lin <- predict(linregmod, val.X, type="response")

library(ROCR)
ROCRfit.lin = prediction(fit.lin, val.y)
ROCRperf.tr.lin = performance(ROCRfit.lin, "tpr", "fpr")

#plot(ROCRperf.tr.ridge, colorize=TRUE, main="Ridge")
plot(ROCRperf.tr.lin,colorize=TRUE, print.cutoffs.at=seq(0,1,by=0.1), main = "ROC-Logistic Regression model",text.adj=c(-0.2,1.7))

#Method using pROC
library(pROC)
lin.roc <- roc(val.y, as.vector(fit.lin) )
coords(lin.roc, "best", best.method="youden", transpose=TRUE)

#Method using visualisation over Grid
# accuracy here used as metric, could use something else
cutoffs <- seq(min(fit.lin),max(fit.lin),(max(fit.lin)-min(fit.lin))/100)
accuracy <- NULL

for (i in seq(along = cutoffs)){
  prediction <- ifelse(fit.lin >= cutoffs[i], "Y", "N") #Predicting for cut-off
